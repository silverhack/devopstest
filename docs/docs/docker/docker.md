...
To run this image, use the ```build.ps1``` script located in the `docker` folder:

``` powershell
.\docker\build.ps1 -Name monkey365 -version latest -Path .\docker\Dockerfile_linux
```
...
``` powershell
.\docker\build.ps1 -Name monkey365 -version latest -Path .\docker\Dockerfile_windows
```
...
``` bash
docker run -it --env-file monkey.env monkey365 pwsh -Command "Invoke-Monkey365"
```
...
docker run -it --env-file monkey.env `
               ...
               monkey365 pwsh -Command "Invoke-Monkey365"
```
